...
def Find_record_intent(conv: V2beta1DialogflowConversation) -> V2beta1DialogflowConversation:
    ...
    #detail = conv.parameters.get("details")
    ...
    Year = conv.contexts.find_record_ctx.parameters["Years"]
    conv.ask(render_template("Record-response", Player=Player, Record=Record, Year=Year))
    conv.google.ask(render_template("Record-response", Player=Player, Record=Record, Year=Year))
    return conv


def Detail_supplementary_intent(conv: V2beta1DialogflowConversation) -> V2beta1DialogflowConversation:
    Details = conv.contexts.find_record_ctx.parameters["Details"]
    conv.ask(render_template("Details-response", Details=Details))
    conv.google.ask(render_template("Details-response", Details=Details))
    return conv
...
def Type_supplementary_intent(conv: V2beta1DialogflowConversation) -> V2beta1DialogflowConversation:
    Type = conv.contexts.find_record_ctx.parameters["Type"]
    conv.ask(Type)
    conv.google.ask(Type)
    return conv
